...
TSharedRef<SDockTab> FMinesweeperToolModule::OnSpawnPluginTab(const FSpawnTabArgs& SpawnTabArgs)
{
    ...
    TSharedRef<SDockTab> DockTab = SNew(SDockTab)
        ...
        .OnTabClosed(SDockTab::FOnTabClosedCallback::CreateRaw(this, &FMinesweeperToolModule::OnPluginTabClosed))
        [
            SNew(SBox)
            .WidthOverride(600)  // Fixed width for consistency
            .HeightOverride(700) // Fixed height for consistency
            [
...
void FMinesweeperToolModule::OnPluginTabClosed(TSharedRef<SDockTab> TabClosed)
{
    // Clear the reference to the tab when it's closed
    MinesweeperTab.Reset();
}
